...
>是一家为医生只能显示片子结果的软件公司 ai算法人数较多， 前端主要调算法接口尽心显示
...
electron 主进程和渲染进程有没有遇到什么问题

项目中有没有什么耦合度比较高的模块（与用户交互比较频繁的）
...
**通过pid和id将arr构建一个tree (第二次遇到这个题了)！！！**
...
vue中触发从新渲染的有哪几种方式  （一直问）

vuex为什么能触发重新渲染呢 （vuex使用了什么技术）

css 一边固定宽度 一边自适应， flot, position ,flex

防抖 （考了n次）

递归处理对象属性   例如 { test_key1 :{ test_key2:[ { test_key3:"test"}] }}  => { testKey1 :{ testKey2:[ { testKey3:"test"}] }} 
