...
![在这里插入图片描述](%E9%A1%B9%E7%9B%AE%E7%90%86%E8%A7%A3/aHR0cHM6Ly91c2VyLWdvbGQtY2RuLnhpdHUuaW8vMjAxOS82LzEwLzE2YjNkMWM5ZDJlMjJhNGE-1608010453393)
...
>
>已选配置的配置列表 通过这个类进行展示 ，主要通过LoadComponents 方法获取
...
>
>没有的话productSave的packSoftwareAndChildren 会报错  ：FCcode  undefinde
  ...
  async initialize() { 								
    ...
    this.swList.forEach((sw, index) => {
      ...
      this.swList[index].a6p = this.a6pList.find(a6p => {
        return a6p.swFCCode === sw.FCCode && a6p.swVersion === sw.swVersion
      })
    })
  }
```
...
var noDiskFCs=["0837"]; 	// 用以判断是否是磁盘索引 （SAN负载源指定）	 return !(fc in noDiskFCs);
var server = new Server;	// 生成server对象

func convert(sr){	        // 规则检查每次先调用这个函数将已选数据 selxxxComponent 转化到server数据中
     if (!sr.checking){
       sr.checking = true;
       convertCPUMem(sr);
       convertFrontMB(sr);
...
![image-20201222092112705](%E9%A1%B9%E7%9B%AE%E7%90%86%E8%A7%A3/image-20201222092112705.png)
...
![image-20201215133833320](%E9%A1%B9%E7%9B%AE%E7%90%86%E8%A7%A3/image-20201215133833320.png)
...
![image-20201215111528918](%E9%A1%B9%E7%9B%AE%E7%90%86%E8%A7%A3/image-20201215111528918-1608010490368.png)
...
constructor(machine) {
    ...
    swList4MES: (2) [{…}, {…}]  软件的解析后的对象
     ...
     mes: [] }
    **/
    this.changeInfo = { hwList: [], swList: [] } //获取mes的changeinfo
    if (this.mode === 'MES') {
      this.sourceInfo = this.grepComponents(this.machine.mesConfig.components)
