...
### 软件部件添加 swComponents 和 hpoComponents 
...
在productSummary  index.vue 中 使用 mergeComponentList 方法过滤了A6P_COMPONENT_ELE 然后放到展示列表中
...
>有根据FCcode生成的硬件和软件列表（hwFCMapList，swFCMapList）和折扣信息列表（fcSpecialDiscountList） 用于更新价格等，包含最新的 productConfig 实例对象
>
